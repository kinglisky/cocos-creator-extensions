...
You should call `event.preventDefault()` if you want to handle this event.
    ...
    on(
      event: 'ready',
      listener: (event: Event, launchInfo: Record<string, any> | NotificationResponse) => void
    ): this
    once(
      event: 'ready',
      listener: (event: Event, launchInfo: Record<string, any> | NotificationResponse) => void
    ...
    on(
      ...
      listener: (event: Event, webContents: WebContents, moduleName: string) => void
    ): this
    once(
      event: 'remote-get-builtin',
      listener: (event: Event, webContents: WebContents, moduleName: string) => void
    ...
    on(
      ...
      listener: (event: Event, webContents: WebContents, globalName: string) => void
    ): this
    once(
      event: 'remote-get-global',
      listener: (event: Event, webContents: WebContents, globalName: string) => void
...
Here's a very simple example of creating a custom Jump List:
     ...
     *
     * By default, web views' cookies and caches will be stored under the `userData`
...
__Note__: On macOS this event is an alias of `move`.
     ...
     * for it to emit properly the 'Swipe between views' preference in `System
...
where `SYSTEM_IMAGE_NAME` should be replaced with any value from this list.
  ...
  interface NativeTheme extends NodeJS.EventEmitter {
     ...
     * * Any UI Electron renders on Linux and Windows including context MenusSidebar,
     * devtools, etc. will use the dark UI.
     * * Any UI the OS renders on macOS including MenusSidebar, window frames, etc. will use
     * the dark UI.
...
e.g.
  ...
  class Session extends NodeEventEmitter {
     ...
     * available to all views in the app with the same `partition`. if there is no
...
Returns an object with system animation settings.
    ...
    /**
     * Whether the Swipe between views setting is on.
     *
     * @platform darwin
     */
...
This value is set to false by default.
  ...
  class WebContents extends NodeEventEmitter {
     ...
     * windows, webviews, opened devtools, and devtools extension background views.
     */
...
The usage is the same with the `login` event of `app`.
     ...
     * @deprecated
     */
    on(event: 'remote-require', listener: (event: IpcMainEvent, moduleName: string) => void): this
    once(event: 'remote-require', listener: (event: IpcMainEvent, moduleName: string) => void): this
    addListener(
      event: 'remote-require',
      listener: (event: IpcMainEvent, moduleName: string) => void
    ): this
...
Calling `event.preventDefault()` will prevent the navigation.
    ...
    once(event: 'will-navigate', listener: (event: Event, url: string) => void): this
    addListener(event: 'will-navigate', listener: (event: Event, url: string) => void): this
    removeListener(event: 'will-navigate', listener: (event: Event, url: string) => void): this
    /**
...
See webContents.sendInputEvent for detailed description of `event` object.
     ...
     * `persist:`, the page will use a persistent session available to all views in the
     * app with the same `partition`. if there is no `persist:` prefix, the page will
     * use an in-memory session. By assigning the same `partition`, multiple views can
  ...
  interface LoadExtensionOptions {
     ...
     * inject content scripts into `file://` views. This is required e.g. for loading
  ...
  interface WebContentsPrintOptions {
    ...
    scaleFactor?: number
    /**
     * The number of views to print per page sheet.
     */
  ...
  interface WebPreferences {
     ...
     * `partition`, multiple views can share the same session. Default is the default
     * session.
     */
    partition?: string
    /**
     * When specified, web views with the same `affinity` will run in the same renderer
     * process. Note that due to reusing the renderer process, certain `webPreferences`
     * options will also be shared between the web views even when you specified
     ...
     * views with the same `affinity`. _Deprecated_
     */
    affinity?: string
    /**
...
declare namespace NodeJS {
  interface Process extends NodeJS.EventEmitter {
     ...
     * performs in-memory compression of views that haven't been recently used. As a
